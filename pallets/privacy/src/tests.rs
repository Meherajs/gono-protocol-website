...
fn test_verify_humanity_proof_success() {
    new_test_ext().execute_with(|| {
        ...
        let attestation = VerifiedAttestations::<Test>::get(&alice(), ProofType::HumanityProof).unwrap();
        assert_eq!(
            attestation,
            Attestation {
...
fn test_verify_credential_verification_success() {
    new_test_ext().execute_with(|| {
        ...
        assert!(NullifierRegistry::<Test>::get(&nullifier));
        let attestation = VerifiedAttestations::<Test>::get(&bob(), ProofType::CredentialVerification).unwrap();
...
fn test_verify_jurisdiction_proof_success() {
    new_test_ext().execute_with(|| {
        ...
        assert!(NullifierRegistry::<Test>::get(&nullifier));
        let attestation = VerifiedAttestations::<Test>::get(&alice(), ProofType::JurisdictionProof).unwrap();
...
fn test_invalid_proof_rejected() {
    new_test_ext().execute_with(|| {
        ...
        assert!(!VerifiedAttestations::<Test>::contains_key(&alice(), ProofType::HumanityProof));
    });
}
...
fn test_revoke_attestation_success() {
    new_test_ext().execute_with(|| {
        ...
        assert!(VerifiedAttestations::<Test>::contains_key(&alice(), ProofType::CredentialVerification));
...
fn test_same_account_multiple_proof_types() {
    new_test_ext().execute_with(|| {
        ...
        assert!(VerifiedAttestations::<Test>::contains_key(&alice(), ProofType::HumanityProof));
        assert!(VerifiedAttestations::<Test>::contains_key(&alice(), ProofType::CredentialVerification));
        assert!(VerifiedAttestations::<Test>::contains_key(&alice(), ProofType::JurisdictionProof));
    });
}
...
fn test_multiple_accounts_same_proof_type() {
    new_test_ext().execute_with(|| {
        ...
        assert!(VerifiedAttestations::<Test>::contains_key(&alice(), ProofType::HumanityProof));
        assert!(VerifiedAttestations::<Test>::contains_key(&bob(), ProofType::HumanityProof));
    });
}
...
fn test_mock_and_failing_zk_verifiers_directly() {
    // Test MockZkVerifier
    assert!(MockZkVerifier::verify(&ProofType::HumanityProof, b"proof", b"inputs"));
    assert!(!MockZkVerifier::verify(&ProofType::HumanityProof, b"", b"inputs"));
    assert!(!MockZkVerifier::verify(&ProofType::HumanityProof, b"proof", b""));

    // Test FailingZkVerifier
    assert!(!FailingZkVerifier::verify(&ProofType::CredentialVerification, b"proof", b"inputs"));
}
