...
fn test_analyst_credit_eq5_eq6() {
    ...
    assert_eq!(reward_reject, calculate_belief_sigmoid(FixedU128::from_u32(1)));
    ...
    assert_eq!(calculate_analyst_credit(FixedU128::zero(), 0, 0), FixedU128::zero());
}
...
fn test_finalize_content_score_threshold_and_timelock() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GonoVerify::finalize_content_score(
                RuntimeOrigin::root(),
                test_cid.clone(),
            ),
...
fn test_finalize_full_pipeline_sanub_end_to_end() {
    new_test_ext().execute_with(|| {
        ...
        assert!(ca_alice > ca_bob, "Approving analyst should have higher score when Bn = 0.8");
