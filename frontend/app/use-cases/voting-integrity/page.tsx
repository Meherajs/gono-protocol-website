    ...
    "timestamp": "2025-11-01T09:15:00Z"
                        ...
                        <section className="mb-12">
                            ...
                            <div className="space-y-6">
                                <div>
                                    <h3 className="text-xl font-semibold text-white mb-4">Voter Registration & DID Issuance</h3>
                                    <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                                        <pre className="text-xs text-gray-300 overflow-x-auto">
{`// Election commission registers eligible voter
import { GonoVoting, zkSNARK, DID } from '@gono/sdk';

const registerVoter = async (voterData) => {
  // Verify eligibility via government database
  const eligible = await verifyNationalID(voterData.nid);
  ...
  const voterDID = await DID.create({
    nidHash: sha256(voterData.nid), // NID never stored plaintext
    constituency: voterData.constituency,
    eligibilityProof: await zkSNARK.generateEligibilityProof({
      age: voterData.age, // Proves age > 18 without revealing exact age
      citizenship: 'Bangladesh',
      residency: voterData.constituency
    })
  });

  // Register DID on blockchain
  const tx = await GonoVoting.registerVoter({
    did: voterDID,
    constituencyCode: voterData.constituency,
    biometricHash: voterData.fingerprintHash, // Stored locally, not on-chain
    publicKey: voterData.publicKey
  });

  console.log(\`Voter registered: \${voterDID}\`);
  console.log(\`Blockchain TX: \${tx.hash}\`);
  
  return { did: voterDID, txHash: tx.hash };
};`}
                                        </pre>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h3 className="text-xl font-semibold text-white mb-4">Anonymous Ballot Casting with zk-SNARKs</h3>
                                    <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                                        <pre className="text-xs text-gray-300 overflow-x-auto">
{`// Voter casts anonymous ballot at polling station
import { GonoVoting, zkSNARK, HomomorphicEncryption } from '@gono/sdk';

const castVote = async (voterDID, candidateChoice) => {
  // Generate zk-SNARK proof of eligibility (without revealing identity)
  const eligibilityProof = await zkSNARK.prove({
    statement: "I am a registered voter who has not voted yet",
    witness: {
      voterDID: voterDID,
      constituency: voterDID.constituency,
      hasVoted: false // Checked against blockchain
    },
    publicInputs: {
      electionID: 'BD-2026-NATIONAL',
      constituencyCode: voterDID.constituency
    }
  });

  // Encrypt vote using homomorphic encryption
  const encryptedVote = await HomomorphicEncryption.encrypt({
    candidateID: candidateChoice,
    electionPublicKey: ELECTION_PUBLIC_KEY
  });

  // Submit anonymous ballot to blockchain
  const tx = await GonoVoting.submitBallot({
    encryptedVote: encryptedVote,
    eligibilityProof: eligibilityProof,
    timestamp: Date.now(),
    nullifier: await zkSNARK.generateNullifier(voterDID) // Prevents double-voting
  });

  // Generate receipt for voter
  const receipt = {
    code: tx.hash.slice(0, 16),
  ...
  console.log('You can verify your vote was counted using this code');
  ...
  return receipt;
                                ...
                                <div>
                                    <h3 className="text-xl font-semibold text-white mb-4">Public Tally Verification & Audit</h3>
                                    <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
                                        <pre className="text-xs text-gray-300 overflow-x-auto">
{`// Anyone can verify election results independently
import { GonoVoting, HomomorphicEncryption, zkSNARK } from '@gono/sdk';

const verifyElectionResults = async (electionID) => {
  console.log('Downloading blockchain election ledger...');
  
  // Download all encrypted votes
  const allVotes = await GonoVoting.getAllVotes(electionID);
  
  console.log(\`Total votes cast: \${allVotes.length}\`);

  // Verify each vote has valid zk-SNARK proof
  let validVotes = 0;
  let invalidVotes = 0;
  
  for (const vote of allVotes) {
    const isValid = await zkSNARK.verify(vote.eligibilityProof);
    if (isValid) {
      validVotes++;
    } else {
      invalidVotes++;
      console.warn(\`Invalid vote detected: \${vote.txHash}\`);
    }
  }
  ...
  // Check for duplicate voters (nullifier uniqueness)
  const nullifiers = allVotes.map(v => v.nullifier);
  const uniqueNullifiers = new Set(nullifiers);
  
  if (nullifiers.length !== uniqueNullifiers.size) {
    console.error('FRAUD DETECTED: Duplicate votes found!');
  } else {
    console.log('✓ No duplicate voters detected');
  }

  // Decrypt homomorphic tally (requires threshold decryption by election officials)
  const encryptedTally = await HomomorphicEncryption.tallyVotes(
    allVotes.map(v => v.encryptedVote)
  );
  
  const results = await HomomorphicEncryption.decrypt(
    encryptedTally,
    ELECTION_PRIVATE_KEY_SHARES // Threshold decryption
  );
  ...
  return {
    ...
    invalidVotes,
    results,
    fraudDetected: invalidVotes > 0 || nullifiers.length !== uniqueNullifiers.size
  };
};

// Run verification
verifyElectionResults('BD-2026-NATIONAL').then(result => {
  if (result.fraudDetected) {
    console.log('⚠️  ELECTION INTEGRITY COMPROMISED');
  } else {
    console.log('✓ ELECTION VERIFIED - RESULTS LEGITIMATE');
  }
});`}
                                        </pre>
                                    </div>
